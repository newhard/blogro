...
//$Id: spVerifyCode.php 1239 2012-07-08 17:25:53Z anythink $ 
...
class spVerifyCode {
	...
	public function  __construct() {
		...
		$this->vc_session = $_SESSION['verifycode'];
	...
	public function display() {
		$this->make_img();
		$_SESSION['verifycode'] = $this->vcode;
		$this->show_img();
		exit();
	}
	...
	public function verify($var, $is_clear = TRUE) {
		$result = FALSE;
		if($var == $_SESSION['verifycode']) {
			$result = TRUE;
		}
		if($is_clear) $_SESSION['verifycode'] = '';
		return $result;
	}
	...
	public function make_img() {
		...
		for($i=0; $i<$this->len; $i++) {
			...
			imagettftext($this->image, $this->textsize, rand(-5,5), $location, rand($this->size-$this->size/10, $this->size+$this->size), $textColor, $this->font, $randtext); //生成单个字体图象
		}
