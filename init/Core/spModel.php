...
//$Id: spModel.php 1235 2012-07-08 16:08:12Z anythink $
...
class spModel {
	...
	public function __construct()
	{
		...
		if(!empty($GLOBALS['G_SP']['db']['sqlmode'])) $this->_db->exec("SET SQL_MODE = '{$GLOBALS['G_SP']['db']['sqlmode']}'");
		$this->yb = $GLOBALS['YB'];
	}
	...
	public function create($row)
	{
		...
		if( FALSE != $this->_db->exec($sql) ){ // 获取当前新增的ID
			if( $newinserid = $this->_db->newinsertid() ){
				...
				$rs = $this->find($row, ($this->pk)?"{$this->pk} DESC":'',$this->pk);
				return array_pop($rs);
			}
		}
		return FALSE;
...
class spPager {
	...
	private function runpager($func_name, $func_args){
		...
		if('findSql'==$func_name){
			$rs = $this->model_obj->findSql("SELECT COUNT(*) as sp_counter FROM ($conditions) sp_tmp_table_pager1");
			$rs = array_pop( $rs ) ;
