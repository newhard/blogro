...
(function($){
 ...
 //(accesskey != undefined ? 'accesskey="' + accesskey + '"' : '') +
      ...
      check = function(id, oldValue, initialCall) {
         ...
         if (!initialCall) {
			  var old_input_value,new_input_value;		  
			  old_input_value = $('#' + opts.prefix + id).val();
			  new_input_value = old_input_value.replace("\\","\\\\");
			  new_input_value = new_input_value.replace("'","\\'");
			
			 checker[id] = setTimeout("check('" + id + "', '" + new_input_value + "', false)", opts.interval);
